...
from plugins.editorial_manager_transfer_service.utils.file_path import create_export_folder
...
from plugins.editorial_manager_transfer_service.utils.settings import (
    get_license_code,
    get_submission_partner_code,
    get_journal_code,
)
from plugins.editorial_manager_transfer_service.utils.transfer_report import (
    get_or_create_transfer_report,
    resolve_transfer_report,
)
from plugins.production_transporter.utilities import data_fetch
from plugins.production_transporter.utilities.file_utils import (
    copy_files_to_temp_deposit_folder,
)
...
def get_article_export_folders() -> str | None:
    """
...
class ExportFileCreation:
    ...
    def __create_export_file(self):
        ...
        self.zip_filepath: str = os.path.join(
        ...
        if self.__get_xml_filepath() is None:
            logger.error(
                logger_messages.process_failed_fetching_metadata(self.article_id)
            )
            self.in_error_state = True
            return
    ...
    def get_submission_partner_code(self) -> str:
        ...
        if not self.__submission_partner_code:
            self.__submission_partner_code: str = get_submission_partner_code(
                self.journal
            )
    ...
    def can_export(self) -> bool:
        ...
        """
        return (
            not self.in_error_state
            and self.article is not None
            and self.journal is not None
            and self.get_license_code() is not None
            and self.get_journal_code() is not None
            and self.get_submission_partner_code() is not None
        )
    ...
    def __create_go_xml_file(
        ...
        go.set(
            consts.GO_FILE_GO_ELEMENT_ATTRIBUTE_XMLNS_XSI_KEY,
            consts.GO_FILE_GO_ELEMENT_ATTRIBUTE_XMLNS_XSI_VALUE,
        )
        go.set(
            consts.GO_FILE_GO_ELEMENT_ATTRIBUTE_SCHEMA_LOCATION_KEY,
        ...
        version: ETree.Element = ETree.SubElement(
            header, consts.GO_FILE_ELEMENT_TAG_VERSION
        )
        version.set(
            consts.GO_FILE_VERSION_ELEMENT_ATTRIBUTE_NUMBER_KEY,
            consts.GO_FILE_VERSION_ELEMENT_ATTRIBUTE_NUMBER_VALUE,
        )
        journal: ETree.Element = ETree.SubElement(
            header, consts.GO_FILE_ELEMENT_TAG_JOURNAL
        )
        journal.set(
            consts.GO_FILE_JOURNAL_ELEMENT_ATTRIBUTE_CODE_KEY, self.get_journal_code()
        )
        import_type: ETree.Element = ETree.SubElement(
            header, consts.GO_FILE_ELEMENT_TAG_IMPORT_TYPE
        )
        import_type.set(
            consts.GO_FILE_IMPORT_TYPE_ELEMENT_ATTRIBUTE_ID_KEY,
            consts.GO_FILE_IMPORT_TYPE_ELEMENT_ATTRIBUTE_ID_VALUE,
        )
        parameters: ETree.Element = ETree.SubElement(
            header, consts.GO_FILE_ELEMENT_TAG_PARAMETERS
        )
        parameter: ETree.Element = ETree.SubElement(
            parameters, consts.GO_FILE_ELEMENT_TAG_PARAMETER
        )
        parameter.set(
            consts.GO_FILE_ATTRIBUTE_ELEMENT_NAME_KEY,
            consts.GO_FILE_PARAMETER_ELEMENT_NAME_VALUE,
        )
        parameter.set(
            consts.GO_FILE_PARAMETER_ELEMENT_VALUE_KEY,
            get_xml_license_code(self.journal),
        )

        # Begin the filegroup.
        filegroup: ETree.Element = ETree.SubElement(
            go, consts.GO_FILE_ELEMENT_TAG_FILEGROUP
        )

        # Create the archive and metadata files.
        archive_file: ETree.Element = ETree.SubElement(
            filegroup, consts.GO_FILE_ELEMENT_TAG_ARCHIVE_FILE
        )
        archive_file.set(
            consts.GO_FILE_ATTRIBUTE_ELEMENT_NAME_KEY, "{0}.zip".format(filename)
        )
        metadata_file: ETree.Element = ETree.SubElement(
            filegroup, consts.GO_FILE_ELEMENT_TAG_METADATA_FILE
        )
        ...
        self.go_filepath = os.path.join(
            self.export_folder, "{0}.go.xml".format(filename)
        )
    ...
    def __fetch_article(
        self, journal: Journal | None, article_id: int | None
    ) -> Article | None:
        """
        ...
        if article is None:
            self.in_error_state = True
            logger.warn(f"Could not fetch article (ID: {article_id}).")
            return None
    ...
    def __fetch_journal(self, janeway_journal_code: str | None) -> Journal | None:
        ...
        logger.debug(
            logger_messages.process_finished_fetching_journal(janeway_journal_code)
        )
    ...
    def log_error(
        ...
        TransferLogs.objects.create(
            report=self.transfer_report,
            journal=self.journal,
            article=self.article,
            message=message,
            message_type=TransferLogMessageType.EXPORT,
            success=False,
        )
